...
for episode in range(number_of_trials):
    ...
    state = tf.concat([ball_position, arm_position, prediction_error], axis=0).numpy().astype(np.float32)
    ...
    if episode % 10000 == 0:
      ...
      agent.actor.save_actor_weights("actor_most_{}.h5".format(episode))
      agent.critic.save_critic_weights("critic_most_{}.h5".format(episode))
    ...
    while not done and ball_index + 1 < ball_path.shape[0]:
        ...
        next_state =  tf.concat([ball_position, arm_position, prediction_error], axis=0).numpy().astype(np.float32)
