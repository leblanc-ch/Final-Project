...
def reward_function_easy(ball_position, arm_position, current_episode, max_episode):
    euc_dist = np.linalg.norm(ball_position - arm_position)

    # The threshold starts high and linearly decays with increasing episode count
    # At episode 250,000 for 500,000 episodes, reaches a minimum limit of 0.032
    # this is the radius of a tennis ball in meters
    # The intention behind this is give the agent an easier time when it knows nothing
    # and slowly make it harder to receive the reward whenever if figures out how to reach the ball
    euc_threshold = 2.0 - ((current_episode)/(max_episode))*2

    if euc_dist < euc_threshold:
        return 0.0, True
    else:
        return -euc_dist, False
...
for episode in range(number_of_trials):
    ...
    ball_position = tf.cast(ball_path[ball_index], tf.float32)
    arm_position = tf.cast(displacement_function(theta1, theta2, theta3, theta4), tf.float32)
    prediction_error = ball_position - arm_position

    state = tf.concat([ball_position, arm_position, prediction_error, 
                   tf.expand_dims(ball_position[0]*arm_position[0], axis=0), 
                   tf.expand_dims(ball_position[0]*arm_position[1], axis=0), 
                   tf.expand_dims(ball_position[0]*arm_position[2], axis=0),
                   tf.expand_dims(ball_position[1]*arm_position[0], axis=0), 
                   tf.expand_dims(ball_position[1]*arm_position[1], axis=0), 
                   tf.expand_dims(ball_position[1]*arm_position[2], axis=0),
                   tf.expand_dims(ball_position[2]*arm_position[0], axis=0), 
                   tf.expand_dims(ball_position[2]*arm_position[1], axis=0), 
                   tf.expand_dims(ball_position[2]*arm_position[2], axis=0)], axis=0).numpy().astype(np.float32)
    ...
    if episode % 10000 == 0:
      ...
      agent.actor.save_actor_weights("actor_positional_features{}.h5".format(episode))
      agent.critic.save_critic_weights("critic_positional_features{}.h5".format(episode))
    ...
    while not done and ball_index + 1 < ball_path.shape[0]:
        ...
        reward, done = reward_function_easy(state[0:3], state[3:6], episode, number_of_trials)
        ...
        next_state =  tf.concat([ball_position, arm_position, prediction_error, 
                   tf.expand_dims(ball_position[0]*arm_position[0], axis=0), 
                   tf.expand_dims(ball_position[0]*arm_position[1], axis=0), 
                   tf.expand_dims(ball_position[0]*arm_position[2], axis=0),
                   tf.expand_dims(ball_position[1]*arm_position[0], axis=0), 
