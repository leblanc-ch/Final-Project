...
def reward_function_easy(ball_position, arm_position, current_episode, max_episode):
    ...
    euc_threshold = 4.0 - ((current_episode)/(max_episode))*4.0
...
for episode in range(number_of_trials):
    ...
    state = tf.concat([ball_position, arm_position], axis=0).numpy().astype(np.float32)
    ...
    if episode % 1000 == 0:
      ...
      agent.actor.save_actor_weights("actor_baseline_{}.h5".format(episode))
      agent.critic.save_critic_weights("critic_baseline_{}.h5".format(episode))
    ...
    while not done and ball_index + 1 < ball_path.shape[0]:
        ...
        next_state =  tf.concat([ball_position, arm_position], axis=0).numpy().astype(np.float32)
