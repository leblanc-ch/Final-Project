...
# Load the weights that correspond to the actor that's going to be tested
actor_one = Actor(6, 4, np.array([0.47124, 0.62832, 0.31416, 0.31416]))
dummy_input = tf.convert_to_tensor(np.zeros((1, 6)), dtype=tf.float32)
...
for file_index, file_name in enumerate(actor_files):
    ...
    while ball_index + 1 < filtered_data.shape[0]:
        ...
        curr_state = np.concatenate((ball_position, arm_position)).astype(np.float32)
        ...
        if ball_was_caught:
            ...
            next_state =  np.concatenate((new_ball_position, new_arm_position)).astype(np.float32)
