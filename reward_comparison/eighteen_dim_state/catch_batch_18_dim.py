...
# Load the weights that correspond to the actor that's going to be tested
actor_one = Actor(18, 4, np.array([0.47124, 0.62832, 0.31416, 0.31416]))
dummy_input = tf.convert_to_tensor(np.zeros((1, 18)), dtype=tf.float32)
...
for file_index, file_name in enumerate(actor_files):
    ...
    while ball_index + 1 < filtered_data.shape[0]:
        ...
        ball_position = filtered_data.iloc[ball_index, 0:3].to_numpy().flatten().astype(np.float32)
        arm_position = displacement_function(theta1, theta2, theta3, theta4).flatten().astype(np.float32)
        prediction_error = np.array([ball_position - arm_position]).flatten().astype(np.float32)

        positional_relations = np.array([
            ball_position[0] * arm_position[0],
            ball_position[0] * arm_position[1],
            ball_position[0] * arm_position[2],
            ball_position[1] * arm_position[0],
            ball_position[1] * arm_position[1],
            ball_position[1] * arm_position[2],
            ball_position[2] * arm_position[0],
            ball_position[2] * arm_position[1],
            ball_position[2] * arm_position[2]
        ], dtype=np.float32)

        curr_state = np.concatenate((ball_position, arm_position, prediction_error, positional_relations)).astype(np.float32)
        ...
        file_total_error += np.linalg.norm(curr_state[0:3] - curr_state[3:6])
        ...
        if ball_was_caught:
            ...
            new_ball_position = filtered_data.iloc[ball_index, 0:3].to_numpy().flatten().astype(np.float32)
            new_arm_position = displacement_function(theta1, theta2, theta3, theta4).flatten().astype(np.float32)
            new_prediction_error = np.array([new_ball_position - new_arm_position]).flatten().astype(np.float32)

            new_positional_relations = np.array([
                ball_position[0] * arm_position[0],
                ball_position[0] * arm_position[1],
                ball_position[0] * arm_position[2],
                ball_position[1] * arm_position[0],
            ...
            next_state =  np.concatenate((new_ball_position, new_arm_position, new_prediction_error, new_positional_relations)).astype(np.float32)
    ...
    print("file: {} with {} total error".format(file_name, file_total_error))
